document.addEventListener("DOMContentLoaded", function () {
  initNavbarAutoClose();
  initTableFilter("#filter-mobil", "#tabel-mobil tbody");
  initTableFilter("#filter-penyewa", "#tabel-penyewa tbody");
  initDeleteConfirm();
  initFormValidation("#form-mobil", mobilValidationRules);
  initFormValidation("#form-penyewa", penyewaValidationRules);
});

function getBasePath() {
  return location.pathname.indexOf("/mobil/") !== -1 ||
...
function toggleEmptyRow(tbody, tampilkan) {
  ...
  if (tampilkan) {
    if (!emptyRow) {
      ...
      emptyRow.innerHTML = "<td colspan='" + kolom + "'>Data tidak ditemukan.</td>";
...
var mobilValidationRules = {
  kode_mobil: { required: true, label: "Kode Mobil" },
  merek_model: { required: true, minLength: 3, label: "Merek & Model" },
  kapasitas: { required: true, min: 2, max: 8, label: "Kapasitas" },
  harga_per_hari: { required: true, min: 100000, label: "Harga per Hari" },
